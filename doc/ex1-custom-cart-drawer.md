...
```
Le cadeau est bien ajouté et supprimé du panier en fonction du montant de celui-ci.
Cela ne fonctionnait pas auparavant car j'avais utilisé un produit non référencé sur la boutique, qui n'était donc pas disponible via l'API de Shopify.
Pour le moment, il faut recharger deux fois la page pour que le cadeau soit ajouté dans le panier.

TODO : 
- Ne pas recharger la page pour ajouter/supprimer le produit du panier
- Déréférencer le cadeau de la boutique
- Récupérer l'ID du cadeau via le snippet personnalisable
- Ajouter condition : si montant cadeau > 0€, ne pas le prendre en compte dans le total du panier
- Ajouter condition dans template : si id cadeau non trouvé ou non défini
